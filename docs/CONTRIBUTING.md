...
| Source Serif 4 | Display, via `font-display` — the wing decides weight and tracking |
| JetBrains Mono | Digits, via `.tabular` — never body text |
...
face. Set the weight and tracking at the call site: COMP takes it semibold and
tight, POKER takes it light, italic and wide.
...
- **POKER** gets two drifting light beams over the `wing-ambient` gradients. In
  `background-image` the first layer is the one on top, which is the trap that
  block warns about: a vignette written last sits under every light in the room.
